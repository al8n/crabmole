...
#[inline]
pub fn append_uvarint(mut buf: alloc::vec::Vec<u8>, x: impl Into<u64>) -> alloc::vec::Vec<u8> {
    let mut x = x.into();
    while x >= 0x80 {
...
#[inline]
pub fn write_uvarint<W: std::io::Write>(mut buf: W, x: impl Into<u64>) -> Result<usize, std::io::Error> {
    let mut x = x.into();
    let mut n = 0;
    while x >= 0x80 {
...
#[cfg_attr(docsrs, doc(cfg(feature = "std")))]
#[inline]
pub fn read_uvarint<R: std::io::Read, I: From<u64>>(mut r: R) -> std::io::Result<(I, usize)> {
    let mut x = 0;
    let mut s = 0;
    for i in 0..MAX_VARINT_LEN64 {
        ...
        if b[0] < 0x80 {
            ...
            return Ok((I::from(x | ((b[0] as u64) << s)), i));
        }
        x |= ((b[0] & 0x7f) as u64) << s;
        s += 7;
    }
...
#[inline]
pub fn put_uvarint(buf: &mut [u8], x: impl Into<u64>) -> usize {
    let mut i = 0;
    let mut x = x.into();
    while x >= 0x80 {
        buf[i] = (x as u8) | 0x80;
...
#[inline]
pub fn uvarint<R: From<u64>>(buf: &[u8]) -> Result<(R, usize), Error> {
    let (mut x, mut s) = (0, 0);
    for (i, b) in buf.iter().enumerate() {
        ...
        if b < 0x80 {
            ...
            return Ok((R::from(x | (b as u64) << s), i + 1));
        }
        x |= ((b & 0x7f) as u64) << s;
        s += 7;
    }
...
#[cfg_attr(docsrs, doc(cfg(feature = "alloc")))]
#[inline]
pub fn append_varint(buf: alloc::vec::Vec<u8>, x: impl Into<i64>) -> alloc::vec::Vec<u8> {
    let x = x.into();
...
#[cfg_attr(docsrs, doc(cfg(feature = "std")))]
#[inline]
pub fn write_varint<W: std::io::Write>(buf: W, x: impl Into<i64>) -> std::io::Result<usize> {
    let x = x.into();
    let mut ux = (x as u64) << 1;
    if x < 0 {
        ux = !ux;
...
#[cfg_attr(docsrs, doc(cfg(feature = "std")))]
#[inline]
pub fn read_varint<R: std::io::Read, I: From<i64>>(r: R) -> std::io::Result<(I, usize)> {
...
/// The buffer is too small. 
#[inline]
pub fn put_varint(buf: &mut [u8], x: impl Into<i64>) -> usize {
    let x = x.into();
    let mut ux = (x as u64) << 1;
    if x < 0 {
        ux = !ux;
...
/// number of bytes read (> 0).
#[inline]
pub fn varint<R: From<i64>>(buf: &[u8]) -> Result<(R, usize), Error> {
